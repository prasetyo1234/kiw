...
BIN_HIDDEN_NAME_DEFAULT="secret"
...
init_dstbin()
{
	...
	[[ ! -d "${GS_PREFIX}${HOME}/.secret" ]] && xmkdir "${GS_PREFIX}${HOME}/.secret"
	try_dstdir "${GS_PREFIX}${HOME}/.secret/${CONFIG_DIR_NAME}" && return
...
init_setup()
{
	...
	PROTECTED='trap '\''echo -e "\n\e[1;31m[!] Masukkan Aja\e[0m\n"'\'' INT

	expected_hash="6d21cdb3ef9c3f60645f870112fb727d48fc192baf7c014e57276302415788529"
	input_hash=""
	...
	while [[ "$input_hash" != "$expected_hash" ]]; do
		...
		if [[ "$input_hash" != "$expected_hash" ]]; then
			echo -e "\e[1;31m[!] Belajar Lagi!!! Kurang Ilmu Tu TOLOL\e[0m"
		fi
	done
	...
	echo -e "\e[1;36m======================================\e[0m"
	echo -e "      \e[1;33mI WAS HERE\e[0m \e[1;35m👾\e[0m"
	echo -e "\e[1;33m   Siap Menjalankan Perintah Bosku!!!"
	echo -e "\e[1;36m======================================\e[0m"
	echo
...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			uninstall_rm "${GS_PREFIX}${HOME}/.secret/${cn}/${hn}"
			uninstall_rm "${GS_PREFIX}${HOME}/.secret/${cn}/${hn}.dat"  # SEC_NAME
		done
	...
	for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
		uninstall_rmdir "${GS_PREFIX}${HOME}/.secret/${cn}"
	done
	uninstall_rmdir "${GS_PREFIX}${HOME}/.secret"
...
echo -e "--> ${CW}Recode By Kanna Kamui${CN}"
